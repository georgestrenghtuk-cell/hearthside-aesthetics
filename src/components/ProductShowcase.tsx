...
const products = [
  ...
  { name: "Pellet Kachel", description: "Ontdek onze selectie pelletkachels voor optimaal comfort.", price: "Bekijk opties", tag: "Nieuw" },
];
    ...
    <section id="producten" className="py-20 bg-muted/50">
      <div ref={ref} className={`container mx-auto px-4 transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
        <div className="text-center mb-14">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-foreground mt-3 mb-4">Premium Pellets voor Elke Kachel</h2>
          <p className="text-muted-foreground max-w-xl mx-auto">Van budget-vriendelijk tot premium — wij hebben de perfecte pellet voor uw situatie.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {products.map((product, i) => (
            <div key={i} className="group relative bg-card rounded-2xl border border-border p-6 hover:border-primary/40 hover:shadow-xl hover:shadow-warm-orange/5 transition-all hover:-translate-y-1">
              {product.tag && (
                <span className="absolute top-4 right-4 bg-primary/10 text-primary text-xs font-bold px-3 py-1 rounded-full">{product.tag}</span>
              )}
